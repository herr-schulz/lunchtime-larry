...
import { CANTEENS, WEEKDAYS, type Diet, type Dish, type Weekday } from "../types.ts";

const DAY_HEAD = /^(montag|dienstag|mittwoch|donnerstag|freitag)\b/i;
const DIET_ONLY = /^(vegan|veggie|vegetarisch|fleisch|fisch|geflügel|schwein|rind|huhn|pute)(\s*\/\s*(vegan|veggie|vegetarisch|fleisch|fisch|geflügel|schwein|rind))?$/i;
const SKIP_DISH = /pizza siehe aushang|änderungen vorbehalten/i;

function weekdayFromHeading(text: string): Weekday | undefined {
...
function dietFromLabel(text: string): Diet | undefined {
...
export async function scrapeBella23(
  ...
  const columns = await page.evaluate(() => {
    ...
    const headings = [...root.querySelectorAll("h3")].filter((h) =>
      /^(montag|dienstag|mittwoch|donnerstag|freitag)/i.test(
        (h.textContent || "").trim(),
      ),
    );
